...
class EnhancedDiagramViewer {
    constructor() {
        ...
        this.edgeStyles = {
            networking:  { color: '#38BDF8', width: 1.8, dash: '',       opacity: 0.75, flowDot: true,  dotColor: '#38BDF8' },
            access:      { color: '#ef4444', width: 1.8, dash: '',       opacity: 0.75, flowDot: true,  dotColor: '#ef4444' },
            data:        { color: '#4ADE80', width: 1.8, dash: '',       opacity: 0.75, flowDot: true,  dotColor: '#4ADE80' },
            trigger:     { color: '#FB923C', width: 2.0, dash: '',       opacity: 0.80, flowDot: true,  dotColor: '#FB923C' },
            dependency:  { color: '#C084FC', width: 1.4, dash: '6,3',   opacity: 0.60, flowDot: false, dotColor: '#C084FC' },
            reference:   { color: '#888888', width: 1.0, dash: '4,4',   opacity: 0.35, flowDot: false, dotColor: '#888888' },
            default:     { color: '#888888', width: 1.4, dash: '',       opacity: 0.50, flowDot: false, dotColor: '#888888' }
        };
        ...
        this.layers = {
            ...
            lambda:           { name: 'Lambda',             color: '#FF9900', resources: [], subgroups: {} },
            apigateway:       { name: 'API Gateway',        color: '#FF9900', resources: [], subgroups: {} },
            elb:              { name: 'Load Balancer',      color: '#FF9900', resources: [], subgroups: {} },
            s3:               { name: 'S3',                 color: '#3F891A', resources: [], subgroups: {} },
            rds:              { name: 'RDS',                color: '#3F48CC', resources: [], subgroups: {} },
            dynamodb:         { name: 'DynamoDB',           color: '#3F48CC', resources: [], subgroups: {} },
            sns:              { name: 'SNS',                color: '#B7CA9D', resources: [], subgroups: {} },
            sqs:              { name: 'SQS',                color: '#B7CA9D', resources: [], subgroups: {} },
            logs:             { name: 'CloudWatch Logs',    color: '#759C3E', resources: [], subgroups: {} },
            cloudwatch:       { name: 'CloudWatch',         color: '#759C3E', resources: [], subgroups: {} },
            iam:              { name: 'IAM',                color: '#FF9900', resources: [], subgroups: {} },
            vpc:              { name: 'VPC',                color: '#248814', resources: [], subgroups: {} },
            route53:          { name: 'Route 53',           color: '#F58536', resources: [], subgroups: {} },
            acm:              { name: 'Certificate Manager',color: '#759C3E', resources: [], subgroups: {} },
            waf:              { name: 'WAF',                color: '#FF9900', resources: [], subgroups: {} },
            secretsmanager:   { name: 'Secrets Manager',    color: '#FF9900', resources: [], subgroups: {} },
            servicediscovery: { name: 'Service Discovery',  color: '#FF9900', resources: [], subgroups: {} }
        };
    ...
    async loadDiagram() {
        ...
        console.log(`Loaded ${this.resources.length} resources and ${this.connections.length} connections`);
    ...
    setupCanvas() {
        ...
        window.addEventListener('resize', debounce(() => this.onResize(), RESIZE_DEBOUNCE_MS));
    ...
    setupControls() {
        ...
        document.getElementById('presentation-mode').addEventListener('click', () => this.togglePresentationMode());
    ...
    setupFilters() {
        const filters = ['type-filter', 'provider-filter', 'region-filter', 'state-filter'];
    ...
    setupKeyboardShortcuts() {
        document.addEventListener('keydown', (e) => {
            ...
            switch (e.key) {
                case 'f': e.preventDefault(); this.fitToView(); break;
                case 'r': e.preventDefault(); this.resetLayout(); break;
                case 'l': e.preventDefault(); this.toggleLabels(); break;
                case 'm': e.preventDefault(); this.toggleMinimap(); break;
                case 'p': e.preventDefault(); this.togglePresentationMode(); break;
                case 'a': e.preventDefault(); this.toggleFlowAnimations(); break;
                case 't': e.preventDefault(); this.toggleSidebar(); break;
                case 'Escape':
    ...
    autoLayout() {
        ...
        const hasLayeredResources = Object.values(this.layers).some(l => l.resources.length > 0);
        ...
        Object.entries(this.layers).forEach(([, layer]) => {
            ...
            subgroupKeys.forEach(sgKey => {
                ...
                const innerH = rows * NODE_SPACING_Y + SUBGROUP_PADDING_Y + SUBGROUP_PADDING_X;
                sgLayouts[sgKey] = { resources: sgResources, cols, rows, innerW, innerH };
            });
            ...
            subgroupKeys.forEach(sgKey => {
                ...
                if (rowX + sg.innerW > LAYOUT_PADDING_X + maxLayerWidth && rowX > LAYOUT_PADDING_X) {
                ...
                sg.resources.forEach((resource, idx) => {
                    ...
                    resource.x = rowX + SUBGROUP_PADDING_X + col * NODE_SPACING_X + NODE_SPACING_X / 2;
                    resource.y = rowY + SUBGROUP_PADDING_Y + row * NODE_SPACING_Y + NODE_SPACING_Y / 2;
                });
    ...
    simpleGridLayout() {
        const resourcesPerRow = Math.max(GRID_MIN_COLS, Math.floor(this.width / GRID_SPACING));
    ...
    createDefs(defsGroup) {
        ...
        Object.entries(this.layers).forEach(([layerKey, layer]) => {
            ...
            const stop1 = svgEl('stop', { offset: '0%', 'stop-color': layer.color, 'stop-opacity': '0.1' });
            const stop2 = svgEl('stop', { offset: '100%', 'stop-color': layer.color, 'stop-opacity': '0.05' });
    ...
    renderLayers(layersGroup) {
        ...
        Object.entries(this.layers).forEach(([layerKey, layer]) => {
            ...
            const minX = Math.min(...visible.map(r => r.x)) - LAYER_BOX_PAD - LAYER_BOX_EXTRA_X;
            const maxX = Math.max(...visible.map(r => r.x)) + LAYER_BOX_PAD + LAYER_BOX_EXTRA_X;
            const minY = Math.min(...visible.map(r => r.y)) - LAYER_BOX_PAD - LAYER_BOX_EXTRA_Y;
            const maxY = Math.max(...visible.map(r => r.y)) + LAYER_BOX_PAD + LAYER_BOX_EXTRA_Y;
            ...
            if (layer.subgroups) {
                Object.entries(layer.subgroups).forEach(([sgKey, sg]) => {
                    ...
                    const sgMinX = Math.min(...sgVisible.map(r => r.x)) - SUBGROUP_BOX_PAD - SUBGROUP_BOX_EXTRA_X;
                    const sgMaxX = Math.max(...sgVisible.map(r => r.x)) + SUBGROUP_BOX_PAD + SUBGROUP_BOX_EXTRA_X;
                    const sgMinY = Math.min(...sgVisible.map(r => r.y)) - SUBGROUP_BOX_PAD - SUBGROUP_BOX_EXTRA_Y;
                    const sgMaxY = Math.max(...sgVisible.map(r => r.y)) + SUBGROUP_BOX_PAD + SUBGROUP_BOX_EXTRA_Y;
    ...
    renderConnections(connectionsGroup) {
        ...
        this.connections.forEach(connection => {
            ...
            if (connection.description) {
                ...
                const labelText = truncateText(connection.description, CONN_LABEL_MAX_CHARS);
                const textWidth = labelText.length * CONN_LABEL_CHAR_WIDTH + CONN_LABEL_PAD;
            ...
            if (style.flowDot && this.showFlowAnimations) {
                ...
                if (dist > FLOW_LONG_DIST) {
                    ...
                    mpath2.setAttributeNS('http://www.w3.org/1999/xlink', 'xlink:href', `#${pathId}`);
    ...
    renderNodes(nodesGroup) {
        this.filteredResources.forEach((resource) => {
            ...
            // Icon
            const iconPath = resource.icon_url || this.getResourceIconPath(resource.type);
            const icon = svgEl('image', {
                href: iconPath,
                x: String(-ICON_OFFSET), y: String(-ICON_OFFSET),
            ...
            if (this.showLabels) {
                ...
                const typeLabel = svgEl('text', {
                    x: '0', y: String(TYPE_LABEL_Y_OFFSET),
                    'text-anchor': 'middle',
                    'font-size': TYPE_LABEL_FONT_SIZE,
                    fill: TYPE_LABEL_COLOR,
                    'font-family': TYPE_LABEL_FONT
                });
                typeLabel.textContent = truncateText(resource.type, TYPE_LABEL_MAX_CHARS);
            ...
            nodeGroup.addEventListener('click', (e) => { e.stopPropagation(); this.selectResource(resource); });
            nodeGroup.addEventListener('mouseenter', (e) => this.showTooltip(e, resource));
    ...
    getResourceIconPath(resourceType) {
        ...
        const iconMap = {
            ...
            'aws:ecs:taskdefinition':               '/icons/Containers/Elastic-Container-Service.svg',
            ...
            'aws:apigateway:basepathmapping':       '/icons/App-Integration/API-Gateway.svg',
            'aws:apigateway:methodresponse':        '/icons/App-Integration/API-Gateway.svg',
            'aws:apigateway:integrationresponse':   '/icons/App-Integration/API-Gateway.svg',
            // Load Balancing
            'aws:elb:loadbalancer':                 '/icons/Networking-Content-Delivery/Elastic-Load-Balancing.svg',
            'aws:elb:listener':                     '/icons/Networking-Content-Delivery/Elastic-Load-Balancing.svg',
            'aws:elb:targetgroup':                  '/icons/Networking-Content-Delivery/Elastic-Load-Balancing.svg',
            'aws:elb:listenerrule':                 '/icons/Networking-Content-Delivery/Elastic-Load-Balancing.svg',
            'aws:elb:targetgroupattachment':        '/icons/Networking-Content-Delivery/Elastic-Load-Balancing.svg',
            // Storage
            ...
            'aws:sns:subscription':                 '/icons/App-Integration/Simple-Notification-Service.svg',
            'aws:sns:topicpolicy':                  '/icons/App-Integration/Simple-Notification-Service.svg',
            ...
            // IAM
            'aws:iam:role':                         '/icons/Security-Identity-Compliance/Identity-and-Access-Management.svg',
            'aws:iam:policy':                       '/icons/Security-Identity-Compliance/Identity-and-Access-Management.svg',
            'aws:iam:user':                         '/icons/Security-Identity-Compliance/Identity-and-Access-Management.svg',
            'aws:iam:rolepolicyattachment':         '/icons/Security-Identity-Compliance/Identity-and-Access-Management.svg',
            // Security
            'aws:acm:certificate':                  '/icons/Security-Identity-Compliance/Certificate-Manager.svg',
            'aws:acm:certificatevalidation':        '/icons/Security-Identity-Compliance/Certificate-Manager.svg',
            'aws:waf:webacl':                       '/icons/Security-Identity-Compliance/WAF.svg',
            'aws:waf:ipset':                        '/icons/Security-Identity-Compliance/WAF.svg',
            'aws:waf:webaclassociation':            '/icons/Security-Identity-Compliance/WAF.svg',
            'aws:secretsmanager:secret':            '/icons/Security-Identity-Compliance/Secrets-Manager.svg',
            'aws:kms:key':                          '/icons/Security-Identity-Compliance/Key-Management-Service.svg',
            // Networking
            'aws:ec2:vpc':                          '/icons/Networking-Content-Delivery/Virtual-Private-Cloud.svg',
            'aws:ec2:subnet':                       '/icons/Networking-Content-Delivery/Virtual-Private-Cloud.svg',
            'aws:ec2:securitygroup':                '/icons/Networking-Content-Delivery/Virtual-Private-Cloud.svg',
            'aws:route53:record':                   '/icons/Networking-Content-Delivery/Route-53.svg',
            'aws:route53:hostedzone':               '/icons/Networking-Content-Delivery/Route-53.svg',
            'aws:cloudfront:distribution':          '/icons/Networking-Content-Delivery/CloudFront.svg',
            // Service Discovery
            'aws:servicediscovery:namespace':       '/icons/Networking-Content-Delivery/Cloud-Map.svg',
            'aws:servicediscovery:service':         '/icons/Networking-Content-Delivery/Cloud-Map.svg',
        };
    ...
    updateMinimap() {
        ...
        this.minimapSvg.setAttribute('viewBox', `0 0 ${MINIMAP_WIDTH} ${MINIMAP_HEIGHT}`);
        ...
        allResources.forEach(resource => {
            ...
            const dot = svgEl('circle', {
                cx: x, cy: y,
                r: isVisible ? String(MINIMAP_DOT_SIZE) : String(MINIMAP_DOT_HIDDEN_SIZE),
    ...
    fitToView() {
        ...
        const minX = Math.min(...this.filteredResources.map(r => r.x)) - FIT_VIEW_MARGIN;
        const maxX = Math.max(...this.filteredResources.map(r => r.x)) + FIT_VIEW_MARGIN;
        const minY = Math.min(...this.filteredResources.map(r => r.y)) - FIT_VIEW_MARGIN;
        const maxY = Math.max(...this.filteredResources.map(r => r.y)) + FIT_VIEW_MARGIN;
        ...
        this.panY = (this.height - contentHeight * this.zoom) / 2 - minY * this.zoom;
    ...
    updateFilters() {
        ...
        const providers = [...new Set(this.resources.map(r => r.provider || 'Unknown'))].sort();
        const regions = [...new Set(this.resources.map(r => r.region || 'Unknown'))].sort();
        ...
        while (providerFilter.firstChild) providerFilter.removeChild(providerFilter.firstChild);
        ...
        while (regionFilter.firstChild) regionFilter.removeChild(regionFilter.firstChild);
    ...
    updateResourceList() {
        ...
        this.filteredResources.forEach(resource => {
            ...
            iconImg.src = this.getResourceIconPath(resource.type);
            iconImg.width = 16;
            iconImg.height = 16;
            iconImg.style.cssText = 'vertical-align:middle; margin-right:6px; flex-shrink:0;';
            ...
            if (resource.tags && Object.keys(resource.tags).length > 0) {
                ...
                Object.entries(resource.tags).slice(0, 3).forEach(([key, value]) => {
    ...
    highlightDependencies(resource) {
        ...
        this.connections.forEach(conn => {
            const pathElement = document.querySelector(`path[data-connection="${conn.id}"]`);
    ...
    clearDependencyHighlighting() {
        document.querySelectorAll('.selected-resource, .upstream-dependency, .downstream-dependency, .unrelated-resource').forEach(el => {
            el.classList.remove('selected-resource', 'upstream-dependency', 'downstream-dependency', 'unrelated-resource');
        });

        document.querySelectorAll('.highlighted-connection, .dimmed-connection').forEach(el => {
    ...
    updateResourceDetailsPanel(resource) {
        ...
        const createDepGroup = (label, items, direction) => {
            ...
            items.forEach(r => {
                ...
                depImg.src = this.getResourceIconPath(r.type);
                depImg.width = 16;
                depImg.height = 16;
        ...
        depsSection.appendChild(createDepGroup('Upstream', connectedResources.upstream, 'upstream'));
        depsSection.appendChild(createDepGroup('Downstream', connectedResources.downstream, 'downstream'));
        ...
        Object.entries(resource.properties || {}).slice(0, 10).forEach(([key, value]) => {
            ...
            propValue.textContent = typeof value === 'string'
    ...
    showTooltip(event, resource) {
        ...
        if (resource.provider) {
            tooltip.appendChild(document.createTextNode(`Provider: ${resource.provider}`));
            tooltip.appendChild(document.createElement('br'));
        }
        if (resource.region) {
            tooltip.appendChild(document.createTextNode(`Region: ${resource.region}`));
            tooltip.appendChild(document.createElement('br'));
        }
        if (resource.state) {
            ...
            if (cloned.hasAttribute('style')) {
                ...
                if (inlineStyle.includes('var(')) {
                    inlineStyle = inlineStyle.replace(/var\(--[^)]+\)/g, (match) => {
                        ...
                        return getComputedStyle(document.documentElement).getPropertyValue(varName).trim() || match;
                    });
    ...
    showError(message) {
        ...
        while (loadingOverlay.firstChild) loadingOverlay.removeChild(loadingOverlay.firstChild);
